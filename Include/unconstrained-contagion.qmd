...
of the adaptive behavior changes in a single time step, in terms of the number
of individuals adopting the adaptive behavior, $A$. $n_t$ will represent
the discrete number of adopters of $A$ at time $t$. 
$n(t)$ is the continuous-valued number of individuals adopting $A$ 
at time $t$ when $t$ is continuous-valued. We close this model subsection by
sketching the agent-based model of diffusion under unconstrained contagion.
