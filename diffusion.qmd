...
In this chapter we start building our framework for social diffusion, represented by mechanistic mathematical and software models of social transmission and learning. We begin with a simple model of a chain of unconstrained social transmission starting from a single teacher, with no limit on how many people might learn from teachers of teachers of teachers, and so on. We also assume behaviors do not require coordination with others, and so the benefit of doing a behavior is independent of the number of people doing the behavior. I show how we can use discrete-time difference equations, continuous-time differential equations, or agent-based simulation to arrive at the same conclusion: prevalence increases exponentially over time under these assumptions, akin to what Rogers might call the "going viral" stage of an emerging trend [@Rogers2003;@Smaldino2023_Ch4Contagion]. 

To understand how real-world constraints affect diffusion, we scaffold from this unconstrained Going Viral model, adding constraints one at a time and observing their effects.  I will first introduce the constraint of *finite populations* in the section *Bounded Virality* below. Next I will explain how the structure of social interactions, represented by social networks, affects diffusion in the *Viral Friends* section. I will close by analyzing how
adaptive social learning constrains diffusion to cultural evolutionary dynamics
in the section *Guided Virality*.
Cultural evolution is driven by adaptive social learning, where individuals are assumed to prefer learning from others who are more successful, for example [@Laland2004;@Kendal2018;@Toyokawa2019]. This example form of adaptive social learning is called *success-biased learning*. Adaptive
learning strategies are to cultural evolution what
preferential mating is to genetic evolution: adaptive social learning induces
evolutionary diffusion dynamics. In the two chapters following this one we will apply this diffusion
framework to cooperative and complementary coordination behaviors, where expected payoffs for different behaviors changes depending on the prevalence of different behaviors.
